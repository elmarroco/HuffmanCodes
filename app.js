...
Heap.prototype = {
  swap: function(i, j) {
    this.items[i] = [this.items[j], (this.items[j] = this.items[i])][0];
  },
...
function HuffmanCodeEncode() {
  ...
  oFReader.onload = function(oFREvent) {
    ...
    var enc = Huffman.encode(oFREvent.target.result);
    download("huffman.txt", enc);
    var dec = Huffman.decode(enc);
    console.log(dec);
  };
}

// Huffman decoding precess
function HuffmanCodeDecode() {
  var oFReader = new FileReader();
  oFReader.readAsText(document.getElementById("uploadText2").files[0]);
